...
<div class="navbar">
    ...
    <ul>
        ...
        <li><a href="statistics.php" role="button" aria-hashed="true"><i class="fa-solid fa-chart-simple" style="margin-right: 10px;"></i>Statistics</a></li>
...
if(isset($_POST['submit'])){
    ...
    if (isset($_POST['book_type']) && $_POST['book_type'] === 'all') {
        $sql = "SELECT * FROM `books` WHERE `book_title` LIKE '%$search%' ORDER BY `uploaded_on` DESC";
    } else if (isset($_POST['book_type']) && !empty($_POST['book_type'])) {
        $book_type = $_POST['book_type'];
        $sql = "SELECT * FROM `books` WHERE `book_title` LIKE '%$search%' AND `book_type` = '$book_type' ORDER BY `uploaded_on` DESC";
    } else {
        $sql = "SELECT * FROM `books` WHERE `book_title` LIKE '%$search%' ORDER BY `uploaded_on` DESC";
    }
} else {
